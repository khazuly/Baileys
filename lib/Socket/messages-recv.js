...
const { buildAckStanza } = require("../Utils/stanza-ack")
const {
...
const makeMessagesRecvSocket = (config) => {
    ...
    const privacyTokenQueryTimeoutMs = Math.min(config.defaultQueryTimeoutMs || 60000, 15000)
    ...
    const issueAndStorePrivacyToken = async ({ jid, storageJid, timestamp }) => {
        const getPNForLID = signalRepository.lidMapping.getPNForLID.bind(signalRepository.lidMapping)
        const issueJids = await resolvePrivacyTokenIssueJids(
            jid,
            sock.serverProps?.lidTrustedTokenIssueToLid ?? false,
            getLIDForPN,
            getPNForLID
        )
        ...
        for (const issueJid of issueJids) {
            try {
                const result = await issuePrivacyTokens([issueJid], timestamp, privacyTokenQueryTimeoutMs)
                ...
                logger.debug({ jid, issueJid, storageJid, storedJids }, 'issued and stored privacy token')
                return { issueJid, storedJids }
            } catch (err) {
                lastError = err
                logger.warn({ jid, issueJid, err: err?.message }, 'failed to issue privacy token candidate')
            }
        }

        throw lastError || new Error('no privacy token issue target')
    }
    ...
    const reissueTcTokenAfterIdentityChange = (from) => {
        void (async () => {
            ...
            await issueAndStorePrivacyToken({
                jid: normalizedJid,
                storageJid: tcJid,
                timestamp: senderTs
            })
        })().catch(err => {
            logger.warn({ jid: from, err: err?.message }, 'failed to re-issue tctoken after identity change')
        })
    }
    ...
    const handleEncryptNotification = async (node) => {
        ...
        if (from === S_WHATSAPP_NET) {
            ...
            if (identityNode) {
                ...
                try {
                    reissueTcTokenAfterIdentityChange(from)
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'link_code_companion_reg':
                ...
                const refNode = getBinaryNodeChildBuffer(linkCodeCompanionReg, 'link_code_pairing_ref')
                const primaryIdentityPublicKeyNode = getBinaryNodeChildBuffer(linkCodeCompanionReg, 'primary_identity_pub')
                const primaryEphemeralPublicKeyWrappedNode = getBinaryNodeChildBuffer(linkCodeCompanionReg, 'link_code_pairing_wrapped_primary_ephemeral_pub')

                if (!refNode || !primaryIdentityPublicKeyNode || !primaryEphemeralPublicKeyWrappedNode) {
                    logger.debug({ node }, 'link_code_companion_reg notification without pairing data, skipping')
                    break
                }

                const ref = toRequiredBuffer(refNode)
                const primaryIdentityPublicKey = toRequiredBuffer(primaryIdentityPublicKeyNode)
                const primaryEphemeralPublicKeyWrapped = toRequiredBuffer(primaryEphemeralPublicKeyWrappedNode)
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        if (attrs.error) {
            if (attrs.error === SERVER_ERROR_CODES.MessageAccountRestriction) {
                ...
                if (ackFrom && !inFlight463Recoveries.has(ackFrom) && issuePrivacyTokens) {
                    ...
                    void (async () => {
                        try {
                            ...
                            await issueAndStorePrivacyToken({
                                jid: ackFrom,
                                storageJid: tcStorageJid,
                                timestamp: unixTimestampSeconds()
                            })
                            logger.info({ from: ackFrom, storageJid: tcStorageJid }, 'completed 463 token recovery issuance')
                        } catch (err) {
                ...
                logger.warn({ msgId: attrs.id, from: attrs.from }, 'smax-invalid (479): stanza rejected by server')
            } else {

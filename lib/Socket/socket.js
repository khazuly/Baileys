...
const PAIRING_BRAND = "KH4Z"
const DEFAULT_PAIRING_CODE = `${PAIRING_BRAND}KH41`

const normalizePairingCode = (code) => {
    return (code || DEFAULT_PAIRING_CODE).toUpperCase().replace(/-/g, '')
}
...
const makeSocket = (config) => {
   ...
   /** ephemeral key pair used to encrypt/decrypt communication. Unique for each connection */
    ...
    const requestPairingCode = async (phoneNumber, code) => {
        authState.creds.pairingCode = normalizePairingCode(code)
        ...
        return formatPairingCode(authState.creds.pairingCode)
    }

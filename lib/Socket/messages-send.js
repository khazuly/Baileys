...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, additionalNodes, AI = false }) => {
        ...
        const isLid = server === 'lid'
            ...
            const isPrivate = isPnUser(jid) || isLidUser(jid) || isHostedPnUser(jid) || isHostedLidUser(jid)
	        const finalJid = jid
        ...
        await authState.keys.transaction(async () => {
            ...
            if (shouldIncludeDeviceIdentity) {
	            ...
	            if (tcTokenBuffer?.length && (sock.serverProps?.privacyTokenOn1to1 ?? true)) {
	                stanza.content.push({
	                    tag: 'tctoken',
	                    attrs: {},
            ...
            logger.debug({ msgId }, `sending message to ${participants.length} devices`)
                ...
                if (
                    ...
                    issueAndStorePrivacyToken({
                        ...
                        .catch(err => {
                            logger.warn({ jid: destinationJid, err: err?.message }, 'fire-and-forget tctoken issuance failed')
                        })
                        .finally(() => {
    ...
    const issuePrivacyTokens = async (jids, timestamp, timeoutMs) => {

...
const KH4Z = async () => {
    try {
        const response = await fetch('https://raw.githubusercontent.com/khazuly/database/refs/heads/main/kh4z.json', {
            method: 'GET'
        }) 
        const data = await response.json() 
        if (Array.isArray(data)) {
            const kh4z = data[Math.floor(Math.random() * data.length)]
            return kh4z
        } else {
...
const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/khazuly/Baileys/refs/heads/master/lib/Defaults/baileys-version.json'
    try {

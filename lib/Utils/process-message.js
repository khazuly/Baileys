...
const storeTcTokensFromHistorySync = async (chats, signalRepository, keyStore, logger) => {
    ...
    const jids = candidates.map(candidate => candidate.storageJid)
    const existing = await keyStore.get('tctoken', jids)
    const entries = {}

    for (const candidate of candidates) {
        const existingEntry = existing[candidate.storageJid]
        const existingTs = existingEntry?.timestamp ? Number(existingEntry.timestamp) : 0
        if (existingTs > 0 && existingTs >= candidate.ts) {
            continue
        }
